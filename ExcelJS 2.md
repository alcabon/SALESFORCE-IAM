...
class ExcelGeneratorEnhanced {
    ...
    static async addFieldMappingSheetWithDropdowns(workbook, extractedFields, salesforceMetadata) {
        ...
        extractedFields.forEach((field, index) => {
            const rowData = [
                ...
                ExcelGeneratorEnhanced.suggestTransformation(field.apiType, field.apiFormat), // Transformation - dropdown
                field.defaultValue || '',
            ...
            row.eachCell((cell, colNumber) => {
                ...
                if (colNumber === 15) {
                    const priority = ExcelGeneratorEnhanced.calculatePriority(field);
                    if (priority === 'HIGH') {
                        cell.fill = {
                            type: 'pattern',
                            pattern: 'solid',
    ...
    static async addAPIConfigurationSheet(workbook, openApiData) {
        ...
        const data = [
            ...
            ['Base_URL', ExcelGeneratorEnhanced.extractBaseUrl(openApiData), 'URL de base', 'YES', 'À configurer selon environnement'],
    ...
    static async addAPISummarySheet(workbook, openApiData, extractedFields) {
        ...
        const stats = ExcelGeneratorEnhanced.calculateAPIStatistics(openApiData, extractedFields);
